...
public sealed class LinuxTray : IDisposable
{
            ...
            _watcherSubscription =
                await _connection.ResolveServiceOwnerAsync(
                    ...
                    onError:
                        ex =>
                            Logger.Error(
                                "StatusNotifierWatcher owner-watch error",
                                ex
                            )
                );
        ...
        ServiceOwnerChangedEventArgs e)
    {
        ...
        {
            Logger.Info(
                "StatusNotifierWatcher owner lost " +
                "(host down or restarting)"
            );
        }
    }
    ...
    private async Task RegisterItemWithWatcherAsync()
    {
        ...
        {
            Logger.Error(
                "Failed to register StatusNotifierItem " +
                "with watcher",
                ex
            );
        }
    ...
    public void Dispose()
    {
        ...
        {
            Logger.Error(
                "Error shutting down Linux tray",
                ex
            );
        }
    }
        ...
        IDBusObject
    {
                ...
                string path)
        {
            ...
            {
                Logger.Error(
                    "Failed to open path via xdg-open: " +
                    path,
                    ex
                );
            }
